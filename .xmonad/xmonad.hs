...
import XMonad.Layout.HintedGrid as HintedGrid
import XMonad.Layout.HintedTile
import XMonad.Layout.HintedTile as HintedTile
...
isConsole           = (className =? "Gnome-terminal")
                 <&&> (stringProperty "WM_WINDOW_ROLE" =? "Scratchpad")
myConsole           = "gnome-terminal -T console -p console --role=Scratchpad"
...
myLayoutHook = --showWorkspaceName
             ...
             $ flex ||| tabs
               ...
               -- ||| HintedGrid.GridRatio (4/3) False ||| HintedTile 1 (1/2) (3/100) HintedTile.Tall ||| HintedTile 1 (1/2) (3/100) HintedTile.Wide
               -- ||| MosaicAlt M.empty ||| mosaic 2 [3,2] ||| multiCol [1] 4 0.01 0.5 ||| OneBig (3/4) (3/4)
               -- ||| ResizableTall 1 (3/100) (1/2) [] ||| Roledex ||| simpleFloat ||| simpleFloat ||| Simplest
               -- ||| spiral (6/7) ||| StackTile 1 (3/100) (1/2) ||| ThreeCol 1 (3/100) (1/2) ||| TwoPane (3/100) (1/2) ||| zoomRow
  where
...
--    myTall = subLayout [] Simplest $ trackFloating (Tall 1 (1/20) (1/2))
    ...
    sGap                = quot gap 2
    myGaps              = gaps [(Gaps.U, gap),   (Gaps.D, gap),   (Gaps.L, gap),   (Gaps.R, gap)]
    mySmallGaps         = gaps [(Gaps.U, sGap),  (Gaps.D, sGap),  (Gaps.L, sGap),  (Gaps.R, sGap)]
    myBigGaps           = gaps [(Gaps.U, gap*2), (Gaps.D, gap*2), (Gaps.L, gap*2), (Gaps.R, gap*2)]
...
myKeys conf = let
    ...
    dirKeys        = ["j","k","h","l"]
    arrowKeys      = ["<D>","<U>","<L>","<R>"]
    dirs           = [ Gaps.D,  Gaps.U,  Gaps.L,  Gaps.R ]
...
--    , ("C-S-<D>"                , addName "Shrink (D on BSP)"           $ tryMsgR (ShrinkFrom U) (MirrorExpand))
      ...
      ("M-["                    , addName "Expand (L on BSP)"           $ tryMsgR (ExpandTowards Gaps.L) (Shrink))
    , ("M-]"                    , addName "Expand (R on BSP)"           $ tryMsgR (ExpandTowards Gaps.R) (Expand))
    , ("M-S-["                  , addName "Expand (U on BSP)"           $ tryMsgR (ExpandTowards Gaps.U) (MirrorShrink))
    , ("M-S-]"                  , addName "Expand (D on BSP)"           $ tryMsgR (ExpandTowards Gaps.D) (MirrorExpand))

    , ("M-C-["                  , addName "Shrink (L on BSP)"           $ tryMsgR (ShrinkFrom Gaps.R) (Shrink))
    , ("M-C-]"                  , addName "Shrink (R on BSP)"           $ tryMsgR (ShrinkFrom Gaps.L) (Expand))
    , ("M-C-S-["                , addName "Shrink (U on BSP)"           $ tryMsgR (ShrinkFrom Gaps.D) (MirrorShrink))
    , ("M-C-S-]"                , addName "Shrink (D on BSP)"           $ tryMsgR (ShrinkFrom Gaps.U) (MirrorExpand))
